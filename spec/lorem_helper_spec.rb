...
RSpec.describe LoremHelper::LoremHelper do
  it 'generates a single word' do
    expect(LoremHelper::LoremHelper.word).to match(/^\w+$/)
  end

  it 'generates multiple words' do
    words = LoremHelper::LoremHelper.words(3)
  ...
  it 'generates a single sentence' do
    ...
    expect(LoremHelper::LoremHelper.sentence).to match(/^[A-Z][a-z]+( [a-z]+){3,9}\.$/)
  end

  it 'generates a single sentence with custom word count' do
    # Specify 12 words (plus 0-6 default random)
    expect(LoremHelper::LoremHelper.sentence(12)).to match(/^[A-Z][a-z]+( [a-z]+){11,17}\.$/)
  end

  it 'generates a single sentence with custom word count and custom random' do
    # Specify 12 words (plus 0 random)
    expect(LoremHelper::LoremHelper.sentence(12, false, 0)).to match(/^[A-Z][a-z]+( [a-z]+){11}\.$/)
  end

  it 'generates multiple sentences' do
    sentences = LoremHelper::LoremHelper.sentences(3)
  ...
  it 'generates a single paragraph' do
    ...
    expect(LoremHelper::LoremHelper.paragraph).to match(/^([A-Z][a-z]+( [a-z]+){2,8}\. ?){3,6}$/)
  end

  it 'generates a single paragraph with custom sentence count' do
    # Specify 12 words (plus 0-6 default random)
    expect(LoremHelper::LoremHelper.paragraph(12)).to match(/^([A-Z][a-z]+( [a-z]+){2,8}\. ?){11,17}$/)
  end

  it 'generates a single paragraph with custom sentence count and custom random' do
    # Specify 12 words (plus 0 random)
    expect(LoremHelper::LoremHelper.paragraph(12, false, 0)).to match(/^([A-Z][a-z]+( [a-z]+){2,8}\. ?){12}$/)
  end

  it 'generates multiple paragraphs' do
    paragraphs = LoremHelper::LoremHelper.paragraphs
    expect(paragraphs).to be_an Array
    expect(paragraphs.size).to eq 3
    paragraphs = LoremHelper::LoremHelper.paragraphs(5)
    expect(paragraphs).to be_an Array
  ...
  it 'generates a single question' do
    # Default sentence is 4 - 10 words
    expect(LoremHelper::LoremHelper.question).to match(/^[A-Z][a-z]+( [a-z]+){3,9}\?$/)
  end

  it 'generates a single question with custom word count' do
    # Specify 12 words (plus 0-6 default random)
    expect(LoremHelper::LoremHelper.question(12)).to match(/^[A-Z][a-z]+( [a-z]+){11,17}\?$/)
  end

  it 'generates a single sentence with custom word count and custom random' do
    # Specify 12 words (plus 0 random)
    expect(LoremHelper::LoremHelper.question(12, false, 0)).to match(/^[A-Z][a-z]+( [a-z]+){11}\?$/)
  end

  it 'generates multiple questions' do
    questions = LoremHelper::LoremHelper.questions(3)
  ...
  it 'generates a valid email' do
    expect(LoremHelper::LoremHelper.email).to match(/^\w+(\.\w+)*@\w+(\.\w+)*\.[a-z]{2,63}$/)
  end

  it 'generates a valid email with custom name' do
    expect(LoremHelper::LoremHelper.email('Test User')).to match(/^(test[_.]user|user[_.]test)@\w+(\.\w+)*\.[a-z]{2,63}$/)
  end

  it 'generates a valid name' do
    expect(LoremHelper::LoremHelper.name).to match(/^([A-Z][a-z]*\.? )?[A-Z][a-z]+ ([A-Z]([a-z]{0,2}|'))?[A-Z][a-z]+(,? \w+\.?)?$/)
  end

  it 'generates a valid first name' do
    expect(LoremHelper::LoremHelper.first_name).to match(/^[A-Z][a-z]+$/)
  end

  it 'generates a valid last name' do
    expect(LoremHelper::LoremHelper.last_name).to match(/^([A-Z]([a-z]{0,2}|'))?[A-Z][a-z]+$/)
  end

  it 'generates a valid title' do
    # Default title is 2 words
    expect(LoremHelper::LoremHelper.title).to match(/^[A-Z][a-z]* [A-Z][a-z]*$/)
  end

  it 'generates a valid title with custom word count' do
    # Default title is 2 words
    expect(LoremHelper::LoremHelper.title(12)).to match(/^[A-Z][a-z]*( [A-Z][a-z]*){11}$/)
  end

  it 'generates a valid date' do
    expect(LoremHelper::LoremHelper.date).to be_a Date
  end

  it 'generates a valid placeholder image' do
    # Default 300x300 PNG
    expect(LoremHelper::LoremHelper.image).to eq 'https://placehold.it/300x300.png'
  end

  it 'generates a valid placeholder image with custom values' do
    expect(LoremHelper::LoremHelper.image('100x200', 'jpeg', '010101', 'FFFFFF', 'Lorem'))
